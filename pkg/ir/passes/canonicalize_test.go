...
func TestCanonicalize_AddConstSecond(t *testing.T) {
	...
	b0 := f.Entry
	arg := f.AddNode(ir.Node{Op: ir.OpLoadArg, Aux: 0, Block: b0})
	f.AppendToBlock(b0, arg)
	c5 := f.AddNode(ir.Node{Op: ir.OpConst, Aux: vm.Int(5), Block: b0})
	f.AppendToBlock(b0, c5)
	// (+ 5 x) — Const first, the form we want to canonicalize away from.
	add := f.AddNode(ir.Node{Op: ir.OpAdd, Refs: []ir.NodeID{c5, arg}, Block: b0})
...
func TestCanonicalize_MulConstSecond(t *testing.T) {
	...
	arg := f.AddNode(ir.Node{Op: ir.OpLoadArg, Aux: 0, Block: b0})
	f.AppendToBlock(b0, arg)
	c3 := f.AddNode(ir.Node{Op: ir.OpConst, Aux: vm.Int(3), Block: b0})
	f.AppendToBlock(b0, c3)
	mul := f.AddNode(ir.Node{Op: ir.OpMul, Refs: []ir.NodeID{c3, arg}, Block: b0})
	f.AppendToBlock(b0, mul)
	ret := f.AddNode(ir.Node{Op: ir.OpReturn, Refs: []ir.NodeID{mul}, Block: b0})
	f.SetTerminator(b0, ret)
...
func TestCanonicalize_NonCommutativeUnchanged(t *testing.T) {
	...
	c5 := f.AddNode(ir.Node{Op: ir.OpConst, Aux: vm.Int(5), Block: b0})
	f.AppendToBlock(b0, c5)
	sub := f.AddNode(ir.Node{Op: ir.OpSub, Refs: []ir.NodeID{c5, arg}, Block: b0})
	f.AppendToBlock(b0, sub)
	ret := f.AddNode(ir.Node{Op: ir.OpReturn, Refs: []ir.NodeID{sub}, Block: b0})
	f.SetTerminator(b0, ret)
...
func TestCanonicalize_BothConstSkipped(t *testing.T) {
	...
	c3 := f.AddNode(ir.Node{Op: ir.OpConst, Aux: vm.Int(3), Block: b0})
	f.AppendToBlock(b0, c3)
	add := f.AddNode(ir.Node{Op: ir.OpAdd, Refs: []ir.NodeID{c5, c3}, Block: b0})
	f.AppendToBlock(b0, add)
	ret := f.AddNode(ir.Node{Op: ir.OpReturn, Refs: []ir.NodeID{add}, Block: b0})
	f.SetTerminator(b0, ret)
...
func TestCanonicalize_BitAndCommutative(t *testing.T) {
	...
	for _, n := range irFn.Nodes {
		...
		lhsOp := irFn.Node(n.Refs[1]).Op
		rhsOp := irFn.Node(n.Refs[2]).Op
