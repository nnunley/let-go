...
// SetVarRoot casts a vm.Value var handle to *vm.Var and SetRoots it; a non-Var
// handle is a no-op. ApplyVarMetaV is the same-shaped guard for metadata.
func TestSetVarRootCastsAndSets(t *testing.T) {
	const ns = "test-setvarroot"
	target := vm.NewNamespace(ns)
	var handle vm.Value = target.Def("x", vm.NIL) // Def returns the *vm.Var
	SetVarRoot(handle, vm.Int(99))
	if got := target.LookupLocal(vm.Symbol("x")); got == nil || got.Deref() != vm.Int(99) {
		t.Fatalf("SetVarRoot did not set root: %v", got)
	}
	SetVarRoot(vm.Int(5), vm.Int(6)) // non-Var: must not panic
}
